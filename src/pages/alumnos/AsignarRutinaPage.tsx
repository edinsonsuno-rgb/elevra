...
export default function AsignarRutinaPage() {
  ...
  function toggleDescanso(dia: DiaKey) {
    setSemana(s => ({
      ...s,
      [dia]: { esDescanso: !s[dia].esDescanso, items: [] }
    }))
  }

  // ── Abrir modal ejercicio ──
  ...
  function agregarEjercicio(ej: CatalogoEj) {
    if (!diaTarget) return
    const item: Item = {
      tempId: uid(), tipo: 'ejercicio', ejercicio: ej,
  ...
  function abrirModalDesc(dia: DiaKey) {
    setDiaTarget(dia)
    setDescMins(2)
    setModalDesc(true)
  }
  ...
  function agregarDescanso() {
    if (!diaTarget) return
    const item: Item = { tempId: uid(), tipo: 'descanso', descanso_minutos: descMins }
    setSemana(s => ({ ...s, [diaTarget]: { ...s[diaTarget], items: [...s[diaTarget].items, item] } }))
    setModalDesc(false)
  }
  ...
  function guardarEditEj() {
    ...
    setSemana(s => ({
      ...
      [editTarget.dia]: {
        ...s[editTarget.dia],
        items: s[editTarget.dia].items.map(i =>
          i.tempId === editTarget.tempId ? { ...i, ...editForm } : i
        )
      }
    }))
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black text-white">Asignar rutina</h1>
          ...
          return (
            <div key={dia}
              className={`df-card overflow-hidden transition-all
                ${data.esDescanso ? 'border-amber-500/30' : ''}`}>
                          ...
                          {data.items.map((item, idx) => (
                            <div key={item.tempId}
                              ...
                              <div className="flex flex-col gap-0.5 flex-shrink-0">
                                <button onClick={() => moverItem(dia, item.tempId, -1)}
                                  disabled={idx === 0}
      ...
      <div className="flex gap-3">
        ...
        <button onClick={guardar} disabled={saving}
          ...
          {saving
            ...
            : <><i className="fa-solid fa-check" /> Guardar rutina</>
          }
        </button>
      </div>
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <input value={busqueda} onChange={e => setBusqueda(e.target.value)}
              placeholder="Buscar ejercicio o músculo..." className="df-input !pl-8 pr-6 text-sm py-2" autoFocus />
          </div>
      ...
      <Modal open={modalDesc} onClose={() => setModalDesc(false)} title="Agregar descanso">
        <div className="space-y-4">
          <p className="text-xs text-df-muted">Define cuántos minutos de descanso entre ejercicios.</p>
